...
import { Search, Tree, Button, Icon, Checkbox } from '@alifd/next';
import { mapCategoryData, getLastLevelData, filterCategoryData } from '../utils';
import './index.less';
  ...
  onCheck = (keys, direction, rightToLeft, callback) => {
    ...
    if (direction === 'left') {
      this.setState(
        {
          leftTree: {
            ...
            checkedKeys: rightToLeft ? this.state.leftTree.checkedKeys : keys,
            // 如果rightToLeft为true时keys是原来的checkedKeys加selectValues，否则为lastLevelKey加selectValues
            keys: rightToLeft ? _.uniq([ ...this.state.selectValues, ...this.state.leftTree.checkedKeys ]) : _.uniq([ ...this.state.selectValues, ...keys ])
          },
        },
        () => {
          const newKeys = _.uniq([...keys, ...this.state.selectValues]);
      ...
      this.setState(
        ...
        () => this.operationOnCheck(keys, this.state.rightTree.dataSource, direction, rightToLeft)
      );
    }
  };
          ...
          {showSearch && (
            <div className="dyx-transfer-search">
              <Search
                ...
                onChange={value => this.handleSearch(value, 'left')}
          ...
          {_.isEmpty(leftTree.dataSource) ? (
            ...
            <div className="dyx-transfer-tree">
              <Tree
                ...
                checkable
                onCheck={keys => this.onCheck(keys, 'left', false)}
          ...
          {showSearch && (
            <div className="dyx-transfer-search">
              <Search
                shape="simple"
                style={{ width: '95%', marginBottom: '10px' }}
                onChange={value => this.handleSearch(value, 'right')}
          ...
          {_.isEmpty(rightTree.dataSource) ? (
            ...
            <div className="dyx-transfer-tree">
              <Tree
                ...
                checkable
                onCheck={keys => this.onCheck(keys, 'right', false)}
                checkedStrategy="child"
